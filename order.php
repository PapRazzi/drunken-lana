...
{	
	...
	if($id > 0)
	{
		...
		$query = "select afunds from members where USERID='".mysql_real_escape_string($_SESSION['USERID'])."'"; 
		$executequery=$conn->execute($query);
		$funds = $executequery->fields['afunds'];
		...
		if($_POST['subbal'] == "1")
		{
			...
			if($funds >= $price)
			{
				$query1 = "UPDATE members SET afunds=afunds-$price WHERE USERID='".mysql_real_escape_string($_SESSION['USERID'])."'"; 

...
elseif($_REQUEST['sortby']=="summ")
{
	$sortby = "email";
	$sort =" order by A.SUMM";
	$add1 = "&sortby=email";
}
else
{
...
$queryselected = "select A.ID, B.username from roboxchange_payments A, members B WHERE A.ID>0 AND A.USER_ID=B.USERID $addtosql $sort $sorthow limit $config[maximum_results]";
$query2 = "select A.*,B.username from roboxchange_payments A, members B WHERE A.ID>0 AND A.USER_ID=B.USERID $addtosql $sort $sorthow limit $pagingstart, $config[items_per_page]";
...
if ($totalposts > 0)
{
	...
	{	
		if($currentpage > 1) 
		{
			$pagelinks.="<a href='$adminurl/payments_robokassa.php?page=1$add1' title='first page'>First</a>&nbsp;";
			$pagelinks.="<a href='$adminurl/payments_robokassa.php?page=$theprevpage$add1'>Previous</a>&nbsp;";
		};
		$counter=0;
		...
		{
			$pagelinks.="<a href='$adminurl/payments_robokassa.php?page=$lowercount$add1'>$lowercount</a>&nbsp;";
			$lowercount++;
			$counter++;
		}
		...
		{
			$pagelinks.="<a href='$adminurl/payments_robokassa.php?page=$uppercounter$add1'>$uppercounter</a>&nbsp;";
			$uppercounter++;
		}
		if($currentpage < $toppage) 
		{
			$pagelinks.="<a href='$adminurl/payments_robokassa.php?page=$thenextpage$add1'>Next</a>&nbsp;";
			$pagelinks.="<a href='$adminurl/payments_robokassa.php?page=$toppage$add1' title='last page'>Last</a>&nbsp;";
		};
	}
}
else
...
STemplate::display("administrator/payments_robokassa.tpl");

...
<script type="text/javascript">
  ...
  var _gaq = _gaq || [];
  _gaq.push(['_setAccount', 'UA-23668210-1']);
...
<script type="text/javascript">     
    VK.init({
        apiId: {/literal}{$VK_APPID}{literal}
    });
</script>
{/literal}
<div id="loadme"></div>
                        ...
                        <div class="holder">
                            ...
                            <div class="menu-c guest">
                                <div id="scriptolutiontopmenu" class="topmenu">
                                    ...
                                    <div style=" margin-left:320px; position:absolute; padding-top:6px;">{if $enable_fc eq "1"}{literal}<fb:login-button perms="email,user_birthday"></fb:login-button>{/literal}{/if}</div>
                                    <div style="margin-left: 420px; position: absolute; padding-top: 6px" id="vk_login_button" onclick="VK.Auth.login(authInfo);"></div> 
                                    {literal}
                                    <script language="javascript">
                                        function authInfo(response) {
                                          if (response.session) {
                                            $.get('/js/vk_login.php', {'prerequest' : 1}, function(data){
                                                if (data == 1) {
                                                    location.reload();
                                                } else {
                                                    var p = 'false';
                                                    while (!validateEmail(p)) {
                                                        p = prompt('Введите корректный адрес электронной почты', '');
                                                    }
                                                    $.get('/js/vk_login.php', {'email' : p}, function(data){
                                                        if (data == 1) {
                                                            location.reload();
                                                        }
                                                    });            

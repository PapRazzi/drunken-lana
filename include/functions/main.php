...
function verify_webmoney_purse($wmPurse) {
    $regExp = "/^R[0-9]{12}$/";
    return preg_match($regExp, $wmPurse);    
}
...
function cleanitreplace($text) 
{
    return preg_replace("/\\s+/"," ", cleanit($text));    
}

function clean_tags($tags)
{
    return preg_replace("/[\\s\\,\\.]+/", " ", trim(cleanit($tags)));
}
			...
			if($price == $rprice)
			{
				if($status != "2" && $status != "3" && $status != "5" && $status != "7")
				{
					...
					if($transid > 0)
					{
						...
						$query = "UPDATE members SET afunds=afunds+$rprice WHERE USERID='".mysql_real_escape_string($buyer)."' limit 1"; 
						$executequery=$conn->execute($query);
					}
				}
			}
